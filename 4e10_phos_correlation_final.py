...
def phos_correlation_4e10(pdb_fp, dcd_fp, phos_index, prefix):
    ...
    for i, frame in enumerate(dcd):
        ...
        #print(len(bb_frame_loop_ext))
        #for i in bb_frame_loop_ext:
        #print(i.getResname())
...
#         frame_ser_OH1 = frame_fab.select( "resnum 242 and name OG" )
...
#         frame_nitrogen_2 = frame_fab.select("resnum 244 and name N")
        ...
        loop_edges = frame_fab.select( "resnum 27 31 and name CA")                 
        frame_ser_OH1 = frame_fab.select( "resnum 28 and name OG" )
...
plt.plot(fr_aln_loop_rmsd_running)
plt.plot(loop_aln_loop_rmsd_running)
...
def plot_phos_correlation(avg_rmsd, stdev_rmsd, po4_occupancy, prefix): 
    ...
    avg_rmsd = avg_rmsd[0:length]
    stdev_rmsd = stdev_rmsd[0:length]

    #x = np.arange(0, len(avg_po4_xyz_RMSD_4e10_n15)*.0002, .0002)
    x = np.arange(0, len(avg_rmsd),1)

    ax1.fill_between(x, -1, 0, where=np.array(po4_occupancy[0:length])==1, 
                     alpha=1, color='#53C767') 
    ax1.fill_between(x, -1, 0, where=np.array(po4_occupancy[0:length])==0, 
